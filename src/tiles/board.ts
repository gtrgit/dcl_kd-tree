...
export class Board extends Entity{
    ...
    constructor(
        ...
        // let ent = new Entity()
        // let hex = new GLTFShape('../../models/hexGreen.glb')
   ...
   raySys(boardPosition: Vector3){
    ...
    class BoardSystems implements ISystem {
      ...
      update(dt: number) {
        if (timer > 0) {
          ...
          timer = .3
          this.handleMove(Camera.instance.feetPosition.x,Camera.instance.feetPosition.z)
          // log(this.t)
        }
      }
      ...
      offset(x:number, z:number){
       ...
       const result:Entity[] = this.tileList.filter(posMatch => posMatch.getComponent(Transform).position.x === x 
      ...
      changeTileToPlayer(changeEnt:Entity[],newTile:GLTFShape){
         ...
         changeEnt.forEach(ent => { ent.addComponentOrReplace(newTile)})
        // for (let i = 0; i < changeEnt.length; i++){
        //  if(changeEnt[i].addComponentOrReplace(newTile)!=undefined){
        //   changeEnt[i].addComponentOrReplace(newTile)
        //  }
        // }
    ...
    drawHexMap(){
        ...
        for (let i = 0; i < this.hexPos.length; i++){
          ...
          // const playerSelection =  new HexTile(this.hexPos[i])
      ...
      placeHexTiles(rows: number, cols: number){
                ...
                for (let x:number = 0; x <cols; x++){
                  ...
                  this.hexTransform  = {position: new Vector3(x+.5, 0, 1.25),scale: new Vector3(.9,1,.9)}
                  ...
                  for (let z = 0; z <rows; z++){
                    ...
                    this.hexTransform  = {position: new Vector3(x, 0, z+1),scale: new Vector3(.9,1,.9)}
                    this.hexPos.push(this.hexTransform)
                    if (this.hexTransform.position){
                      this.points.push(`${this.hexTransform.position.x} ${this.hexTransform.position.z}`)
                      this.tree.insert([this.hexTransform.position.x, this.hexTransform.position.z])
                    }
                  }
                ...
                for (let x = 0; x <cols; x++){
                  ...
                  this.hexTransform  = {position: new Vector3(x, 0, 1),scale: new Vector3(.9,1,.9)}
                  this.hexPos.push(this.hexTransform)
                  ...
                  this.hexTransform  = {position: new Vector3(x+.5, 0, y+1),scale: new Vector3(.9,1,.9)}
                  this.hexPos.push(this.hexTransform)
                  if(this.hexTransform.position) {
                    this.points.push(`${this.hexTransform.position.x} ${this.hexTransform.position.z}`)
                    this.tree.insert([this.hexTransform.position.x, this.hexTransform.position.z])
